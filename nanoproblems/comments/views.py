...
def new_comment(request):
    if request.method == "POST":
        comment = logic.createcomment(request)
...
def show_comment(request, comment_id):
    comment = logic.checkcommentexists(comment_id)
...
def edit_comment(request, comment_id):
    ...
    if request.method == 'POST':
        ...
        logic.editcomment(request, comment)
        return HttpResponseRedirect('/comments/show_comment/' + str(comment.id))
...
def delete_comment(request, comment_id):
    comment = logic.checkcommentexists(comment_id)
    if not logic.deletecomment(request, comment):
